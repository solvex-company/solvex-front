...
function InfoDashboard() {
  ...
  const fullName = user ? `${user.name} ${user.lastname}` : `Nombre del usuario`;
  return (
    <section className="grid grid-cols-2 justify-center items-center w-full gap-3 py-6">
      ...
      <div>
        ...
        <h3 className="border border-accent rounded-lg p-1">
          {user ? user.identification_number : `Numero de identificacion del usuario`}
        </h3>
      </div>
      <div>
        <h2>Numero de Telefono</h2>
        <h3 className="border border-accent rounded-lg p-1">
          {user ? user.phone : `Numero de telefono del usuario`}
        </h3>
      </div>
    </section>
  );
