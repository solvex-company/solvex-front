    ...
    <div className="w-[967px]">
      <section className="flex justify-between pb-5">
        <div className="flex justify-start gap-4">
          ...
          <div className="w-max">
            <h3>Fecha</h3>
            <p className="border border-accent rounded-md w-[223px] p-2">{formatDate(ticket.creation_date)}</p>
          </div>
        </div>
        <div>
          <h3>Estado</h3>
          <p className={`text-center text-white rounded-md w-[194px] p-2 ${getStatusColor(ticket.id_status.name)}`}>
        ...
        <div className="border border-accent rounded-md flex justify-center items-center gap-5 p-5">
          {(() => {
            ...
            return images.length ? (
              ...
              <p className="italic text-gray-500">No se adjunto ninguna imagen a este ticket.</p>
            );
          })()}
        </div>
      </div>
