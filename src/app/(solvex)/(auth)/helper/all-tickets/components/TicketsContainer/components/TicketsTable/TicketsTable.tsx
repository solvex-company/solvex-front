      ...
      <table className="flex w-full flex-col h-max gap-2 mt-5 table-fixed">
        <thead>
          <tr className="w-full flex justify-between px-5 py-2 font-semibold">
            ...
            <th className="w-3/6 text-left text-lg">Titulo</th>
            <th className="w-1/6 text-left text-lg">Fecha</th>
        ...
        <tbody className="flex flex-col gap-3">
          {tickets.map((ticket) => {
            return (
              ...
              >
                <td className="w-1/6">{ticket.id}</td>
                <td className="w-3/6">{ticket.title}</td>
                <td className="w-1/6">{ticket.date}</td>
                <td className="w-1/6">{ticket.employee}</td>
              </tr>
            );
          })}
        </tbody>
